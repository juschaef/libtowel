													...
													void *next, void *output)
{
	...
	if (next)
	{
		twl_lst_push_back(output, JSON_STR_SEP);
	}
}
																 ...
																 void *output)
{
	...
	{
		twl_lst_push_back(output, "\"");
		twl_lst_push_back(output, key);
		twl_lst_push_back(output, "\":");
	}
	twl_json_dump_rec(node, output);
	if (next)
...
void				twl_json_dump_rec(t_jnode *node, t_lst *output)
{
	...
	else if (node->type == JSON_OBJECT)
	{
		...
		twl_lst_push_back(output, twl_itoa(twl_jnode_get_primitive(node)));
	else if (node->type == JSON_BOOL)
		twl_lst_push_back(output, twl_jnode_get_primitive(node) ? "true" : "false");
	else if (node->type == JSON_NULL)
		twl_lst_push_back(output, JSON_STR_NULL);
