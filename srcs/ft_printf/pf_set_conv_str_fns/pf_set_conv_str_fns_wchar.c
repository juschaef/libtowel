...
char	*pf_conv_str_wstr(void *val)
{
	...
	return (ft_wstr_to_str((wchar_t *)(val)));
}

char	*pf_conv_str_wchar(void *val)
{
	return (free_val(ft_wstr_to_str((wchar_t *)(val)), val));
}

char	*pf_conv_str_hex_up(void *val)
{
	return (ft_strupcase(pf_conv_str_hex(val)));
}

char	*pf_conv_str_hex_ul_up(void *val)
{
	return (ft_strupcase(pf_conv_str_hex_long(val)));
}

char	*pf_conv_str_unsign(void *val)
{
	return (free_val(ft_lltoa(VOID_PTR_TO(unsigned int, val)), val));
}
