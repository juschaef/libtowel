...
{
	return (free_val(ft_itoa(VOID_PTR_TO(int, val)), val));
}

char *pf_conv_str_short(void *val)
{
	return (free_val(ft_itoa(VOID_PTR_TO(short int, val)), val));
}

char *pf_conv_str_ushort(void *val)
{
	return (free_val(ft_itoa(VOID_PTR_TO(unsigned short int, val)), val));
}

char *pf_conv_str_octal_us(void *val)
{
	return (free_val(ft_llutobasestr(VOID_PTR_TO(unsigned int, val), 8), val));
}
