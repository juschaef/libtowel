...
static void			set_err_msg(char *msg, char *key, t_argparser_result *result)
{
	twl_asprintf(&(result->err_msg), "%s: %s", msg, key);
}

static void			handle_has_option_argument(char *key, t_argparser_result_item	*result_item, t_argparser_result *result, t_lst *tokens_to_consume)
{
...
void				argparser_parse_build_argument__(char *key, t_argparser_result *result, t_lst *tokens_to_consume)
{
	...
	argument = argparser_argument_mgr_find_by_key(result->argparser->arguments, key);
	if (argument)
	{
