...
static void multiple_replacements(t_test *test)
{
	...
	out = ft_str_replace(s, "aaa", "xxx");
	out = ft_str_replace(out, "bbb", "www");
	out = ft_str_replace(out, "111", "888");
	out = ft_str_replace(out, "333", "999");
	mt_assert(strcmp(out, "xxxwwwcccxxxwwwccc888222999888222999") == 0);
}

void	suite_ft_str_replace(t_suite *suite)
{
