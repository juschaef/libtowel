...
static void test_set_without_del(t_test *test)
{
	t_lst *lst;

	lst = twl_lst_new();
	twl_dict_set(lst, "key1", strdup("data1"), NULL);
	twl_dict_set(lst, "key2", strdup("data2"), NULL);
	mt_assert(strcmp(twl_lst_get_elem__(lst, 0)->data, "data1") == 0);
	mt_assert(twl_lst_len(lst) == 2);
	twl_dict_set(lst, "key1", strdup("new data1"), NULL);
	// mt_assert(strcmp(twl_lst_get_elem__(lst, 0)->data, "new data1") == 0);
	// mt_assert(twl_lst_len(lst) == 2);
}
...
void	suite_twl_dict_set(t_suite *suite)
{
	...
	SUITE_ADD_TEST(suite, test_set_without_del);
}
